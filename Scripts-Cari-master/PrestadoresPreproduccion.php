...
class PrestadoresPreproduccion
{
    ...
    public function process(\Phalcon\Mvc\Micro $app)
    {
        ...
        $nameController = "PrestadoresPreproduccion";
        ...
        if ($useProduction && md5($token) == 'da0c25bc1d54b8961680edf08ab03ea6') {
            switch ($operation) {
                ...
                case "AuthBonoCero":
                    ...
                    echo $res;
                    break;
                case "localAuthBonoCero":
                    $res = $this->localAuthBonoCero($app, $params_error_report, $nameController, $chat_id);
                    echo $res;
                    break;
                case "getRoleRips":
    ...
    public function ListDoctor($app, $params_error_report, $nameController, $chat_id, $urlIn)
    {
        ...
        $ciclo = $_POST['ciclo'];
        $nomDocBusqueda = $_POST['nomDocBusqueda'];
        $nomDocBusqueda = strtoupper($nomDocBusqueda);
        ...
        if (count($response_object->relacion) == '0') {
            ...
            for ($i = 0; $i < $longitud; $i++) {
                ...
                $nombrePrestador = $response_object->relacion[$i]->prestador->razonSocial;
                if (strpos($nombrePrestador, $nomDocBusqueda) !== false && 
                    $response_object->relacion[$i]->prestador->tipoId !== '4' && 
                    !in_array($response_object->relacion[$i]->prestador->numId, $listadoNumId)) {
    ...
    public function ConsultarAutorizacionNumeroV2($app, $params_error_report, $nameController, $chat_id, $urlIn)
    {
        ...
        if (count($response_object->authorization[0]->procedure) > 1) {
            ...
            $descripcionBH = "";
            $uvrAutorizacion = "";
            $bilateralidadAutorizacion = "";
            $codigoEquipoMedicoAutorizacion = "";
            $sesionesNew = "";
            for ($i = 0; $i < count($response_object->authorization[0]->procedure); $i++) {
                if ($i < (count($response_object->authorization[0]->procedure) - 1)) {
                    ...
                    $bilateralidadAutorizacion = $bilateralidadAutorizacion . ($response_object->authorization[0]->procedure[$i]->interventionType->code == '1' || $response_object->authorization[0]->procedure[$i]->interventionType->code == '3' || $response_object->authorization[0]->procedure[$i]->interventionType->code == '4' ? "1" : "0") . '-';
                    $codigoEquipoMedicoAutorizacion = $codigoEquipoMedicoAutorizacion . $response_object->authorization[0]->procedure[$i]->performer->actor->practitionerRole->code . '-';
                } else {
                    $codigoBh = $codigoBh . $response_object->authorization[0]->procedure[$i]->identifier[0]->value;
                    $descripcionBH = $descripcionBH . $response_object->authorization[0]->procedure[$i]->identifier[1]->value;
                    $uvrAutorizacion = $uvrAutorizacion . $response_object->authorization[0]->procedure[$i]->numberUVR;
            ...
            $bilateralidadAutorizacion = $bilateralidadAutorizacion . ($response_object->authorization[0]->procedure[0]->interventionType->code == '1' || $response_object->authorization[0]->procedure[0]->interventionType->code == '3' || $response_object->authorization[0]->procedure[0]->interventionType->code == '4' ? "1" : "0");
            $codigoEquipoMedicoAutorizacion = $codigoEquipoMedicoAutorizacion . $response_object->authorization[0]->procedure[0]->performer->actor->practitionerRole->code;
        }
        ...
        return '{
		        ...
		        "codigoBH":"' . $codigoBh . '",
		        "bilateralidad":"' . $bilateralidadAutorizacion . '",
                "codigoEquipoMedicoAutorizacion": "' . $codigoEquipoMedicoAutorizacion . '",
                "tipoPrestacionQuirurgica": "' . $response_object->authorization[0]->category->code . '",
                ...
                "servicios": [
                    {
                        ...
                        "tipoPrestacion":"' . $codigo_servicio_new . '"
                    }
                ],
            "pines": [
                {
...
//         $fechaUtilizacion = $_POST['fechaUtilizacion'];
    ...
    public function localAuthBonoCero($app, $params_error_report, $nameController, $chat_id, $numAuth, $tipoConsulta, $codPrestacion)
    {
        $nameFunction = "localAuthBonoCero()";
        $useProduction = $_POST['useProduction'];
        ...
        if ($errorCode->errorCode == "OK") {
            ...
            if (gettype($procedimientos2) == 'array') {
                for ($j = 0; $j < count($procedimientos2); $j++) {
                    if ($procedimientos2[$j]->codPrestacionOSI == $codPrestacion) {
                        return $procedimientos2[$j]->valorTotalNegociado;
                    }
                }
            } else {
...
//print_r(gettype($procedimientos2));
    ...
    private function grabarAtencionAuthV2($app, $params_error_report, $nameController, $chat_id, $urlIn)
    {
        ...
        $codigo_diagnostico = $_POST['codigoDiagnostico'];
        $observaciones = $_POST['observaciones'];
        $bilateralidad = $_POST['bilateralidad'];
        $codigoEquipoMedicoAutorizacion = $_POST['codigoEquipoMedicoAutorizacion'];
        $tipoPrestacionQuirurgica = $_POST['tipoPrestacionQuirurgica'];
        ...
        if ($space_position) {
            ...
            $codigoEquipoMedicoAutorizacionArray = explode("-", $codigoEquipoMedicoAutorizacion);
            $bilateralidadArray = explode("-", $bilateralidad);
            ...
            for ($i = 0; $i < $contador; $i++) {
                ...
                $valorDelServicio = $this->localAuthBonoCero($app, $params_error_report, $nameController, $chat_id,$numero_solicitud,1,$codigoServicioArray[$i]);
                if ($i < ($contador - 1)) {
                    if ($compania == 30 || $compania == 31) {
                        $existe_valor = "null";
                    ...
                    if ($tipoPrestacionQuirurgica == "5") {
                        $servicios = $servicios . '
                        {
                            ...
                            "codigoOrigenAutorizacion": ' . $codigo_origen_autorizacion . ',
                            "valorConvenio": ' . $valor_convenio . ',
                            "bilateral": ' . $bilateralidadArray[$i] . ',
                            "codigoEquipoMedico": ' . $codigoEquipoMedicoAutorizacionArray[$i] . ',
                            "tipoPrestacion": ' . $tipoPrestacionQuirurgica . '
                            },
                            ';
                    } else {
                        $servicios = $servicios . '
