...
class MtController {
        ...
        if ($useProduction) {
            switch ($operation) {
                ...
                case "crearErrorLog":
                    ...
                    break;
                case "getToken":
                    $response = $this->getTokenWs(false);
                    echo $response;
                    break;
                case "getResponseToken":
                    $response = $this->getTokenWs(true);
                    echo $response;
                    break;
                case "recordarContra":
    ...
    private function getTokenWs($getResponseToken = false) {

        //$token = ColsanitasApiToken::getToken($this->useProduction, $this->session->bot_in_use);
        ...
        try {
            ...
            if ($token) {
//                $tokobj = new ColsanitasApiToken();
//                $tokobj->token = $token;
//                $tokobj->expires_in = $obj->expires_in;
//                $tokobj->production = 0;
//                if ($this->useProduction) {
//                    $tokobj->production = 1;
//                }
//                $tokobj->ctime = date("Y-m-d H:i:s", strtotime("+$tokobj->expires_in seconds"));
//                $tokobj->flux_bot_id = $this->session->bot_in_use;
//                $tokobj->safeSave();
